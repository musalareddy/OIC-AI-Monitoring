import os

from pydantic_settings import BaseSettings
from pydantic import Field


class Settings(BaseSettings):
	oic_base_url: str = Field(..., alias="OIC_BASE_URL")
	oic_instance_name: str | None = Field(None, alias="OIC_INSTANCE_NAME")
	oauth_token_url: str = Field(..., alias="OAUTH_TOKEN_URL")
	oauth_client_id: str = Field(..., alias="OAUTH_CLIENT_ID")
	oauth_client_secret: str = Field(..., alias="OAUTH_CLIENT_SECRET")
	oauth_scope: str | None = Field(None, alias="OAUTH_SCOPE")
	mcp_log_file: str = Field("mcp_server.log", alias="MCP_LOG_FILE")

	http_timeout_secs: int = Field(30, alias="HTTP_TIMEOUT_SECS")
	http_max_retries: int = Field(2, alias="HTTP_MAX_RETRIES")

	class Config:
		# Allows running two instances of this same codebase (e.g. Test and
		# Prod) side by side: set OIC_ENV_FILE to point each process at its
		# own .env file instead of duplicating the repo.
		env_file = os.getenv("OIC_ENV_FILE", ".env")
		case_sensitive = False


settings = Settings() 